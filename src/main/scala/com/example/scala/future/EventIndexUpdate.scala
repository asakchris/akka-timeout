...
class EventIndexUpdate(implicit system: ActorSystem) {
  ...
  def updateEventIndexProc(eventIndex: EventIndex, status: Int, errorMessage: Option[String]) = {
    log.info(s"Before calling updateEventIndex proc: ${eventIndex.eventId}, indexId: ${eventIndex.indexId}")
    Future { 1 }
  }

  def execute(eventIndex: EventIndex, status: Int, eventIndexWorkerActor: ActorRef) = {
